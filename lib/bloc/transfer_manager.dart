...
class TransferManager{
  ...
  Stream<TransferWrapper> get transferWrapper async* {
    ...
    List<Transfer> transferSentList = await _transferService.readBySender(userPhone);
    List<Transfer> transferReceivedList = await _transferService.readByReceiver(userPhone);

    TransferWrapper transferWrapper = new TransferWrapper();
    transferWrapper.transferSentList = await _transferService.sortDescending(transferSentList);
    transferWrapper.transferReceivedList = await _transferService.sortDescending(transferReceivedList);
